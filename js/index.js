...
var Utl, addHistory, init, initGraph, initOther, initTimes, n2filename, n2p, play, refreshTotal, sumHigh, sumLow;
...
refreshTotal = function() {
  ...
  $('.times').each(function() {
    ...
    id = $(this).data('id');
    if (id === 'total') {
      return $(this).html(Utl.arraySum(window.times));
    } else {
      return $(this).html(window.times[id]);
    }
  });
...
addHistory = function(hash) {
  var historyTr, isHigh, isHighStr, j, len, n, p, pStr, record, ref, table, tr;
  ...
  for (j = 0, len = ref.length; j < len; j++) {
    ...
    if (isHigh !== null && p !== null) {
      ...
      pStr = p !== null ? Math.round(p * 1000) / 10 : '';
      tr.append($('<td>').html('<strong>' + isHighStr + '</strong><br>' + '→<br>' + '<small>' + pStr + '%</small>').addClass('center'));
    }
...
initOther = function() {
  return $('#history_ok_min').html(window.CONST_HISTORY_OK_MIN);
};
...
initGraph = function() {
  ...
  $('#result').append(tr);
  tr = $('<tr>').append($('<th>').html('合計').addClass('title')).append($('<td>').addClass('times').data('id', 'total')).append($('<td>').addClass('persent').data('id', 'total')).append($('<td>').addClass('graph_cell'));
  $('#result').append(tr);
  results = [];
  for (index = j = 0; j <= 53; index = ++j) {
    tr = $('<tr>').append($('<th>').html(index).addClass('title')).append($('<td>').addClass('times').data('id', index)).append($('<td>').addClass('persent').data('id', index)).append($('<td>').append($('<img>').attr('src', 'img/graph.png').addClass('graph').data('id', index)).addClass('graph_cell'));
